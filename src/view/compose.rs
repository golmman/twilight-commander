...
impl Pager {
    ...
    fn get_dir_prefix(&self, path_node: &PathNode) -> String {
        let expanded_char = if self.config.composition.use_utf8 {
            '▼'
        } else {
            'v'
        };

        let reduced_char = if self.config.composition.use_utf8 {
            '▶'
        } else {
            '>'
        };
        
        let expanded_indicator = if path_node.is_expanded { expanded_char } else { reduced_char };
